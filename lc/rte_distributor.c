...
int *wkr_table1 = NULL;   // fill in rte_distributor_process 
int *wkr_table2 = NULL;
int *wkrs1_connections = NULL;
int *wkrs2_connections = NULL;
...
adder_inf(int num_workers) 
{
	...
	wkrs1_connections = calloc(first_wkrs_num, sizeof(int));
	wkrs2_connections = calloc(second_wkrs_num, sizeof(int));
}
...
free_inf() {
	...
	free(wkr_table1);
	free(wkr_table2);
	free(wkrs1_connections);
	free(wkrs2_connections);
}
...
find_worker1(int flowlet_id)
{
	...
	if (new_flowlet) {
		...
		int min_val = -1;
		for (int i = 0; i < first_wkrs_num; ++i) {
			if (min_val == -1 || min_val > wkrs1_connections[i]) {
				min_ind = i;
				min_val = wkrs1_connections[i];
			}
		}
		wkr = min_ind;
...
find_worker2(int flowlet_id)
{
	...
	if (need_just2 >= 0) {
                ...
                int min_val = -1;
                for (int i = 0; i < second_wkrs_num; ++i) {
                        if (min_val == -1 || min_val > wkrs2_connections[i]) {
                                min_ind = i;
                                min_val = wkrs2_connections[i];
                        }
                }
                wkr = min_ind;
