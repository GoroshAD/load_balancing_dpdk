...
//----------//for the backgroud process
#include <sys/time.h>
#include <sys/types.h>
#include <math.h>
//----------

int **flowlet_table = NULL;   // structure: [flowlet_id, wkr1, wkr2, wpa (was pkts ago)]
int flowlet_number = 0;
int first_wkrs_num = 0;
int second_wkrs_num = 0;
int *wkr_table1 = NULL;
int *wkr_table2 = NULL;
unsigned int wkr1;
unsigned int wkr2;
int DELTA = 9;   //delta in flowlet definition
pid_t background_proc = NULL;   //background proc's id
struct timeval tp;
long long int time_now = 0;
long long int time_was = 0;
long long int time_last_pkt = 0;
...
print_flowlet_table() {
        ...
        for (int i = 0; i < flowlet_number; ++i) {
                printf("%d: %d %d\n", flowlet_table[i][0], flowlet_table[i][1], flowlet_table[i][2]);
        }
        printf("-----------------------------\n");
}
...
int
find_worker1(struct rte_distributor *d, int flowlet_id) 
{
	//printf("start\n");
	//int func1 = func_1(flowlet_id);
	...
	int ret_wkr = 0;
        ...
        if (new_flowlet) {
                ...
                flowlet_table[flowlet_number - 1][0] = flowlet_id;
                gettimeofday(&tp, NULL);
                time_last_pkt = tp.tv_sec * 1000 + tp.tv_usec / 1000;
		flowlet_table[flowlet_number - 1][3] = time_last_pkt;
		while (unlikely(!d->active[wkr1]))
			wkr1 = (wkr1 + 1) % first_wkrs_num;
		ret_wkr = wkr1;
        	flowlet_table[flowlet_number - 1][1] = ret_wkr;
		flowlet_table[flowlet_number - 1][2] = first_wkrs_num + 1;
	}

        //wkr_table1[ret_wkr] += func1; // to release
	//printf("finish\n");
        return ret_wkr;
}

int
find_worker2(struct rte_distributor *d, int flowlet_id)
{
	//printf("start\n");
        //int func2 = func_2(flowlet_id);
	...
	bool new_flowlet = true;
        ...
        for (int i = 0; i < flowlet_number; ++i) {
                if (flowlet_table[i][0] == flowlet_id) {   //find
                        ...
                        time_last_pkt = tp.tv_sec * 1000 + tp.tv_usec / 1000;
                        flowlet_table[i][3] = time_last_pkt;
                        if (flowlet_table[i][2] == first_wkrs_num + 1) {
                                need_just2 = i;
                                break;
                        }
                        ret_wkr = flowlet_table[i][2];
                        new_flowlet = false;
                        break;
                }
        }
	if (need_just2 >= 0) {
		while (unlikely(!d->active[wkr2 + first_wkrs_num]))
                	wkr2 = (wkr2 + 1) % second_wkrs_num;
                ret_wkr = wkr2;
                flowlet_table[need_just2][2] = ret_wkr;
	} else if (new_flowlet) {
		++flowlet_number;
                flowlet_table = realloc(flowlet_table, flowlet_number * sizeof(int *));
                flowlet_table[flowlet_number - 1] = calloc(FIELDS, sizeof(int));
		...
		while (unlikely(!d->active[wkr2 + first_wkrs_num]))
                        wkr2 = (wkr2 + 1) % second_wkrs_num;
                ret_wkr = wkr2;
                flowlet_table[flowlet_number - 1][2] = ret_wkr;
	}
        //printf("emm?\n");

        //wkr_table2[ret_wkr - first_wkrs_num] += func2; // to release
        //printf("finish\n");
        return ret_wkr + first_wkrs_num;
}
		 ...
		 */
			...
			int flowlet_id_wkrs = 0;
                        if (new_tag % 2 == 0) {
                                ...
                                //new_tag += 2;
                                flowlet_id_wkrs = find_worker1(d, new_tag);
                        } else {
                                //mbufs[next_idx++]->hash.usr = next_mb->hash.usr - 1;
                                //--new_tag;
                                //printf("here");
                                flowlet_id_wkrs = find_worker2(d, new_tag);
                        }
                        struct rte_distributor_backlog *bl =
                        ...
                        if (unlikely(bl->count ==
                                        RTE_DIST_BURST_SIZE)) {
                                release(d, flowlet_id_wkrs);
                                if (!d->active[flowlet_id_wkrs]) {
                                        j--;
                                        next_idx--;
                                        matching_required = 1;
                                        continue;
