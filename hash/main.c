...
lcore_distributor(struct lcore_params *p)
{
	...
	while (!quit_signal_dist) {
		...
		if (nb_rx) {
			...
			for (int i = 0; i < nb_rx; ++i) {   //first pkts will be even
				//printf("%lld\n", bufs[i]->hash.usr);
				if (bufs[i]->hash.usr % 2 == 1) {
					bufs[i]->hash.usr = bufs[i]->hash.usr + 1;
				}
				//printf("%d %lld\n", (uint16_t)bufs[i]->hash.usr, bufs[i]->hash.usr);
			}
			...
			//-----------------------------------------------------------
			int i, sec = 0;
			struct rte_mbuf *second_bufs[BURST_SIZE * 4];
			for (i = 0; i < nb_ret; ++i) {
				if (bufs[i]->hash.usr % 2 == 0) {
					second_bufs[sec] = bufs[i];
					++sec;
					bufs[i]->hash.usr = bufs[i]->hash.usr - 1;
				}
			}
			if (sec > 0) rte_distributor_process(d, second_bufs, nb_rx);
			//-----------------------------------------------------------
