...
int 
flid2hash1(int flowlet_id, int i) {
	return (flowlet_id % 10 + flowlet_table[i][1]) % 3;
}
...
find_worker1(int flowlet_id)
{
	...
	for (int i = 0; i < flowlet_number; ++i) {
		if (flowlet_table[i][0] == flowlet_id) {   //find
			...
			flowlet_table[i][2] = time_last_pkt;
			return flid2hash1(flowlet_id, i);
		}
	}
	++flowlet_number;
        flowlet_table = realloc(flowlet_table, flowlet_number * sizeof(int *));
        flowlet_table[flowlet_number - 1] = calloc(3, sizeof(int));
	flowlet_table[flowlet_number - 1][0] = flowlet_id;
	flowlet_table[flowlet_number - 1][1] = 0;
        gettimeofday(&tp, NULL);
        time_last_pkt = tp.tv_sec * 1000 + tp.tv_usec / 1000;
        flowlet_table[flowlet_number - 1][2] = time_last_pkt;
	int ret_wkr = flid2hash1(flowlet_id, flowlet_number - 1);
	//printf("finish1\n");
	return ret_wkr;
}
...
find_worker2(int flowlet_id) {
	...
	for (int i = 0; i < flowlet_number; ++i) {
                if (flowlet_table[i][0] == flowlet_id) {   //find
                        ...
                        time_last_pkt = tp.tv_sec * 1000 + tp.tv_usec / 1000;
                        flowlet_table[i][2] = time_last_pkt;
                        return flid2hash2(flowlet_id, i);
                }
        }
        ++flowlet_number;
        flowlet_table = realloc(flowlet_table, flowlet_number * sizeof(int *));
        ...
        time_last_pkt = tp.tv_sec * 1000 + tp.tv_usec / 1000;
        flowlet_table[flowlet_number - 1][2] = time_last_pkt;
        int ret_wkr = flid2hash2(flowlet_id, flowlet_number - 1);
	//printf("finish2\n");
	return ret_wkr;
}
...
table_update()
{
	...
	gettimeofday(&tp, NULL);
        ...
        for (int i = 0; i < flowlet_number; ++i) {
                if (abs(time_now - flowlet_table[i][2]) <= DELTA) {   //addition
                        flowlet_table[i][1] = flowlet_table[i][1] + 1;
                }
        }
	//printf("finish_t\n");
	return;
